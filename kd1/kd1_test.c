...
#define TRANSLATE_BUF_SIZE 4096
...
static unsigned char translate_buff[TRANSLATE_BUF_SIZE];
...
static int handle_translate_map(const char* filename)
{
    ...
    ssize_t total=0, n;
    while ((n=read(fd,translate_buff+total,TRANSLATE_BUF_SIZE-total))>0) {
        total+=n;
        if (total==TRANSLATE_BUF_SIZE) {
