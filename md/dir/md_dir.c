...
static void dfs(const char* path, Vector* all_files);
...
int main(int argc,char** argv) 
{
    ...
    char cwd[MAX_PATH];
    getcwd(cwd,MAX_PATH);
    Vector* files=create_vector(100);
    dfs(cwd,files);
    //for (int i=0;i<files->len;i++) puts(files->elements[i]->name);
    return 0;
}
...
static bool group_matches_file(const Group* g, const File* f)
{
    if (!g || !f) return false;
...
static HashTable* create_hash_table()
{
    ...
    ht->groups = calloc(bucket_count, sizeof(Group*));
    if (!ht->groups) {
        free(ht);
...
static void hash_table_insert_file(HashTable* hash_table,File* f)
{
    ...
    while (g) {
        if (get_group_hash(g) == h && group_matches_file(g, f)) {
            push_file_vector(&g->files, f);
            return;
        }
        g = g->next_group;
...
static void dfs(const char* path, Vector* all_files)
{
    ...
    while ((entry=readdir(dir))!=NULL) {
        ...
        struct stat st;
        if (is_dir(abs_path)) {
            dfs(abs_path,all_files);
            continue;
        }
        // Is file?
        if (S_ISREG(st.st_mode)) {
            File* f=create_file(abs_path, &st);
            if (f) push_file_vector(all_files,f);
        }
    }
    closedir(dir);
}
